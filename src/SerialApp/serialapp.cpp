...
void SerialApp::onUICmd(quint16 cmd, const CmdParam *params, quint32 num)   //???
{
...
void SerialApp::parseData(const quint8* data, int len)
{
    ...
    qint16 tmp = ptr[index++];  //32767
    qint32 IO1 = (tmp<<16 & 0xffff0000)|ptr[index++]; //开关量   //0
    emit setIO1Param(IO1);//其他IO量

    qint16 tmp1 = ptr[index++];  //32767
    qint32 IO2 = (tmp1<<16 & 0xffff0000)|ptr[index++]; //开关量   //0
    emit setIO2Param(IO2);//四个样本通道IO量

    qint16 alarmType = ptr[index++];
    emit setAlarmType(alarmType);

    qint16  Readcode = ptr[index++];
    emit setReadcode(Readcode);
